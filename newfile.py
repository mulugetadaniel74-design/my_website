...
@app.route('/')
def home():
    content = f"<div style='padding:50px;'><img src='{my_photo}' style='width:150px; border-radius:50%; border: 4px solid #004d40;'><h1>Welcome to Daniel's Grand Hotel</h1><a href='/register' style='background:#ffcc00; color:black; padding:15px 40px; text-decoration:none; border-radius:30px; font-weight:bold;'>GET STARTED</a></div>"
...
@app.route('/register')
def register():
    # ዳንኤል፣ ያንተን ሊንክ እዚህ ጋር አስተካክዬ አስገብቼዋለሁ
    content = """
    <div style='padding: 20px;'>
        <div style='background:white; max-width:400px; margin:auto; padding:30px; border-radius:20px; box-shadow:0 10px 20px rgba(0,0,0,0.1); text-align:left;'>
            ...
            <form action="https://formspree.io/f/xlgwjnee" method="POST">
                ...
                <button type='submit' style='width:100%; background:#004d40; color:white; padding:15px; border:none; border-radius:8px; font-weight:bold; cursor:pointer;'>SEND TO DANIEL</button>
            </form>
        </div>
    </div>
    """
